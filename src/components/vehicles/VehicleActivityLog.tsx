...
import { Gauge, ClipboardCheck, History } from "lucide-react";
...
export function VehicleActivityLog({ vehicleId, variant = "card" }: VehicleActivityLogProps) {
  const { data: logs, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Fetch mileage logs and inspections in parallel
      const [mileageRes, inspectionRes] = await Promise.all([
        supabase
          .from("mileage_logs")
      ...
      for (const i of inspectionRes.data) {
        ...
        const statusLabel = i.status === "completed" ? "Suoritettu" : i.status === "overdue" ? "Myöhässä" : "Aloitettu";
        entries.push({
          id: `i-${i.id}`,
          type: "inspection",
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <Badge variant="outline" className="text-[10px] px-1.5 py-0">
                    {entry.type === "mileage" ? "Kilometrit" : "Tarkastus"}
                  </Badge>
